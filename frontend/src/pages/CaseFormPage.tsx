...
export default function CaseFormPage() {
  ...
  const [customer, setCustomer] = useState<any>({
    lastNameKana: '', firstNameKana: '', phone: '', email: '', postalCode: '',
    ...
    if (state?.customer) {
      setCustomer((c: any) => {
        ...
        return {
          ...c,
          ...sc,
          lastNameKana: sc.lastNameKana || sc.nameKana || c.lastNameKana || '',
          firstNameKana: sc.firstNameKana || c.firstNameKana || '',
  ...
  const submit = async (e?: FormEvent) => {
    e?.preventDefault();
    if (saving || !customer.lastNameKana || !customer.phone) return;
    setError('');
    setSaving(true);
    try {
      const caseInput: any = {};
  ...
  return (
    <div>
      ...
      <div className="card">
        <h3>顧客情報</h3>
        <div className="grid3">
          <Field label="苗字（カナ）"><input value={customer.lastNameKana} onChange={cf('lastNameKana')} placeholder="ヤマダ" /></Field>
          <Field label="名（カナ）"><input value={customer.firstNameKana} onChange={cf('firstNameKana')} placeholder="タロウ" /></Field>
      ...
      <button type="submit" disabled={saving || !customer.lastNameKana || !customer.phone}>
        {saving ? '登録中...' : '案件を登録'}
      </button>
      </form>
    </div>
