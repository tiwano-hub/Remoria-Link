...
router.get('/', async (_req, res) => {
  const [taxRates, channels, sources, locations, stores] = await Promise.all([
  ...
  res.json({ taxRates, channels, sources, locations, stores });
});
...
// ---- 汎用マスタ（販路・反響経路・保管場所） ----
function simpleMaster(model: 'salesChannel' | 'referralSource' | 'storageLocation') {
  const r = Router();
  r.post('/', manage, async (req, res) => {
    const { name } = req.body as { name?: string };
    if (!name) return res.status(400).json({ error: 'name が必要です' });
    const created = await (prisma[model] as any).create({ data: { name } });
    res.status(201).json(created);
  });
  r.delete('/:id', manage, async (req, res) => {
    await (prisma[model] as any).update({ where: { id: req.params.id }, data: { active: false } });
    res.json({ ok: true });
  });
  return r;
}
